---
name: amtb-sync
description: Sync the AMTB raw corpus (amtb/doc/) with amtb.tw. Use when the user asks to "sync the input /doc folder", "sync doc", "refresh sources", "update corpus", "sync from amtb.tw", or "run the source update". Triggers on keywords like sync, doc, corpus, amtb.tw, fetch, download, manifest.
---

# Sync AMTB raw corpus (doc/)

Updates `amtb/doc/` from amtb.tw. Run from the **`amtb/` directory** — all tools use CWD-relative paths for `menu.json` and `doc/`.
...
Run these steps in order, from `C:\Users\Long\Documents\amtb\amtb`:
